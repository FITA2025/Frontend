...
public class AnchorManager : MonoBehaviour
{
    ...
    public GameObject inputCanvas;         // anchorNUM 입력을 위한 WorldSpace Canvas
    public TMP_InputField anchorNUMInputField; // 가상 키보드와 연결될 입력창
    ...
    public string currentAnchorTYPE = "way";
    public string currentFireDt = null; // 초기 발화 여부 상태
    ...
    void ShowInputUI(Vector3 anchorPosition)
    {
        ...
        anchorNUMInputField.text = "";
        // anchorNUMInputField.Select(); // 입력 필드 포커스 (정상작동 시 삭제할 것)
        anchorNUMInputField.ActivateInputField();
    ...
    public void OnInputComplete()
    {
        ...
        {
            string finalName = anchorNUMInputField.text;
            if (string.IsNullOrEmpty(finalName)) finalName = "0";
    ...
    async Task LoadAndResolveAnchorsAsync()
    {
        ...
        foreach (var unbound in unboundBuffer)
        {
            ...
            // [추가] JSON 데이터에서 이 앵커의 uuid에 해당하는 anchorNUM 찾기
            var record = wrapper.anchors.Find(a => a.uuid == spatialAnchor.Uuid.ToString());
            if (record != null)
            {
                anchorNames[spatialAnchor] = record.anchorNUM.ToString(); // 딕셔너리 복구
                var textLabel = anchorObj.GetComponentInChildren<TextMeshProUGUI>();
                if (textLabel != null) textLabel.text = record.anchorNUM.ToString(); // 텍스트 표시
            }
        }
    }
    ...
    {
        // 1. 기존 JSON 데이터 로드 (수동 수정된 속성값 보존용)
        Dictionary<string, AnchorRecord> existingDataLookup = new Dictionary<string, AnchorRecord>();
        if (File.Exists(SavePath))
        {
            try
            {
                string existingJson = File.ReadAllText(SavePath);
                AnchorListWrapper existingWrapper = JsonUtility.FromJson<AnchorListWrapper>(existingJson);
                if (existingWrapper?.anchors != null)
                {
                    foreach (var record in existingWrapper.anchors)
                    {
                        if (!string.IsNullOrEmpty(record.uuid))
                            existingDataLookup[record.uuid] = record;
                    }
                }
            }
            catch (Exception e)
            {
                Debug.LogError($"[JSON] 기존 데이터 로드 중 오류: {e.Message}");
            }
        }
        ...
        {
            string uuidStr = anchor.Uuid.ToString();
            anchorNames.TryGetValue(anchor, out string rawNum); // InputField에서 받은 string

            // string을 int로 변환
            int.TryParse(rawNum, out int resolvedNUM);

            // 2. 기존 데이터가 있는지 확인 (UUID 기준)
            if (existingDataLookup.TryGetValue(uuidStr, out AnchorRecord existing))
            {
                // [기존 앵커] JSON에서 수동으로 수정한 floor, roomId, fire_dt는 그대로 유지
                // anchorNUM은 가상 키보드로 방금 수정했을 수 있으므로 최신 resolvedNUM 사용
                anchorMetadataList.Add(new AnchorRecord {
                    uuid = uuidStr,
                    floor = existing.floor,        // 기존 값 보존
                    roomId = existing.roomId,      // 기존 값 보존
                    anchorNUM = resolvedNUM,       // 이름은 최신값 반영
                    anchorTYPE = existing.anchorTYPE,
                    FireDt = existing.FireDt     // 기존 값 보존
                });
            }
            else
            {
                // [신규 앵커] 이번에 처음 생성된 앵커이므로 현재 관리자 설정값 적용
                anchorMetadataList.Add(new AnchorRecord {
                    uuid = uuidStr,
                    floor = currentFloor,
                    roomId = currentRoomId,
                    anchorNUM = resolvedNUM,
                    anchorTYPE = currentAnchorTYPE,
                    FireDt = currentFireDt ?? "N"
                });
            }
        }
        ...
        Debug.Log($"[JSON] 병합 저장 완료: {SavePath}");
    }
}
