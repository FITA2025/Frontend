...
public class AnchorManager : MonoBehaviour
{
    ...
    void ShowInputUI(Vector3 anchorPosition)
    {
        ...
        inputCanvas.transform.LookAt(Camera.main.transform);
        ...
        if (virtualKeyboard != null)
        {
            ...
            // Meta "Near" 설정값 복제: 사용자 앞 0.4m, 아래 0.4m
            Vector3 targetPos = camTransform.position + (camTransform.forward * 0.4f) + (camTransform.up * -0.4f);
            virtualKeyboard.transform.position = targetPos;
            
            virtualKeyboard.transform.LookAt(camTransform);
            virtualKeyboard.transform.Rotate(65f, 180f, 0f); // 65도 눕히기
