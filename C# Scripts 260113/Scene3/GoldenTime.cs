...
//                             이때 Scene #1의 AppStateManager(DontDestroyOnLoad)에 저장된 serverBaseUrl/playerId가 존재하면
//                             Inspector 값(baseApiUrl/userId) 대신 AppState 값을 우선 적용한다.
...
//                                   - AppStateManager가 존재하고 값이 채워져있다면, Start()에서 AppState 값으로 덮어써서 사용한다.
//      userId (int)               : (Inspector) 서버 DB의 user.id 값. /calculate/{user_id} 경로에 사용.
//                                   - AppStateManager가 존재하고 playerId > 0이면, Start()에서 AppState 값으로 덮어써서 사용한다.
...
public class ServerManagerGoldenTime : MonoBehaviour
{
    ...
    // 외부(UI 게이지 등)로 골든타임 값을 전달하기 위한 이벤트
    public System.Action<int, int> OnGoldenTimeChanged;

    // 시작(서버 수신) 시점의 max 골든타임(초)
    private int _maxGoldenTimeSeconds = -1;
    ...
    //             시작 시 Scene #1(AppStateManager)에 저장된 serverBaseUrl/playerId가 존재하면
    //             Inspector 값(baseApiUrl/userId) 대신 AppState 값을 우선 적용.
    // 입력 파라미터 : 없음
    // 리턴 타입 : void
    private void Start()
    {
        // Scene #1(AppStateManager)에서 입력받은 서버 URL / Player ID를 우선 적용한다.
        // - AppStateManager가 없으면(예: Scene #3 단독 실행) Inspector 값(baseApiUrl/userId)을 그대로 사용한다.
        ApplyAppStateIfPresent();
    ...
    // 함수 이름 : ApplyAppStateIfPresent()
    // 함수 기능 : Scene #1에서 DontDestroyOnLoad로 유지되는 AppStateManager가 존재하면
    //            거기서 입력받은 serverBaseUrl / playerId를 baseApiUrl / userId에 반영.
    //            AppStateManager가 없으면(예: Scene #3 단독 실행) Inspector 값(baseApiUrl/userId)을 그대로 사용.
    //            AppStateManager 값이 존재하면, Start()에서 한 번만 덮어쓴 후 기존 로직(PollLoop/GetGoldenTimeOnce)을 그대로 수행.
    // 입력 파라미터 : 없음
    // 리턴 타입 : void
    private void ApplyAppStateIfPresent()
    {
        // AppStateManager는 Scene #1에서 생성되어 Scene #2/#3로 넘어와도 유지되는 전역 설정 저장소.
        // AppState가 없으면 Inspector 값을 그대로 사용.
        var app = AppStateManager.I;
        if (app == null) return;

        if (!string.IsNullOrWhiteSpace(app.serverBaseUrl))
            baseApiUrl = app.serverBaseUrl.Trim();

        // AppState의 playerId는 Scene #1에서 이미 정규화(예: C046118 -> 946118)되어 저장돼있음.
        if (app.playerId > 0)
            userId = app.playerId;
    }
    ...
    {
        _maxGoldenTimeSeconds = Mathf.Max(startSeconds, 1);

        _currentDisplayedSeconds = Mathf.Max(startSeconds, 0);
        SetText(_currentDisplayedSeconds.ToString());

        // UI 게이지에 "초기값(가득 참)" 전달
        OnGoldenTimeChanged?.Invoke(_currentDisplayedSeconds, _maxGoldenTimeSeconds);
    ...
    private IEnumerator LocalCountdownLoop()
    {
        while (_currentDisplayedSeconds > 0)
        {
            ...
            // UI 게이지에 현재값 전달
            OnGoldenTimeChanged?.Invoke(_currentDisplayedSeconds, Mathf.Max(_maxGoldenTimeSeconds, 1));
